...
public class KDTreeNode<T extends KDComparable> extends KDTree<T> {
    ...
    protected T nearest(ParsedNode query, boolean sortByLon, T currentBest) {
        ...
        if(sortByLon) {
            ...
            else nearestPossibleLT = Util.calcDistance(query, new ParsedNode(leftSplit, query.getLat()));

            if(query.getLon() > rightSplit) nearestPossibleRB = 0;
            else nearestPossibleRB = Util.calcDistance(query, new ParsedNode(rightSplit, query.getLat()));
        } else {
            if(query.getLon() < leftSplit) nearestPossibleLT = 0;
            else nearestPossibleLT = Util.calcDistance(query, new ParsedNode(query.getLon(), leftSplit));

            if(query.getLat() > rightSplit) nearestPossibleRB = 0;
            else nearestPossibleRB = Util.calcDistance(query, new ParsedNode(query.getLon(), rightSplit));
        }
        
        if(nearestPossibleLT == 0) {
            candidate = (T)leftChild.nearest(query, !sortByLon, best);
        }
        if(nearestPossibleRB == 0) {
        	candidate = (T)rightChild.nearest(query, !sortByLon, best);
        }
