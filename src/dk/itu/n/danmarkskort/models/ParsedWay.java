...
public class ParsedWay extends ParsedItem implements Serializable{
    ...
    public Shape getPath() { return getPath(true); }
    public Shape getReversedPath() { return getPath(false); }
    @Override
    public void deleteOldRefs() { nodes = null; }
    @Override
    public void makeShape() { shape = getPath(); }
    @Override
    ...
    private Shape getPath(boolean rightWay) {
        if(shape != null) return shape;
            ...
            if (rightWay) {
                ...
                path.moveTo(nodes[nodes.length - 1].getLon(), nodes[nodes.length - 1].getLat());
